class E9Crm::PageViewsController < E9Crm::ResourcesController
  ...
  def collection_scope
    ...
    scope = scope.joins(<<-SQL.sub(/\s+/, ' '))
      LEFT OUTER JOIN campaigns
        ON campaigns.id = page_views.campaign_id
    SQL
  ...
  def collection
    get_collection_ivar || begin
      ...
      objects       = collection_scope.paginate(pagination_parameters.merge(:total_entries => total_entries))
