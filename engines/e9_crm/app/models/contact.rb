class Contact < ActiveRecord::Base
  ...
  has_many :dated_costs
  has_many :costs, :class_name => "DatedCost", :conditions => { :credit => false }
  ...
  scope :search, lambda {|query|
    ...
    where_sql = any_attrs_like_scope_conditions(:first_name, :last_name, :title, :info, query)
  ...
  scope :subscribed_to, lambda {|*lists|
    ...
    select("distinct contacts.*").joins(:users => :subscriptions).where('subscriptions.mailing_list_id' => lists)
  }
  ...
  scope :tagged, lambda {|tags, options={}|
    if tags.present?
      tagged_with tags, options.reverse_merge(:show_hidden => true)
    else
      where("1=0")
    end
  }
  ...
  def create_user
    users.create(self.class.users_build_parameters)
  end
  ...
  def merge_tags(taggable_or_tags)

    other_tags = if taggable_or_tags.respond_to?(:tag_list_on)
      taggable_or_tags.tag_list_on('users__h__')
    else
      taggable_or_tags
    end
