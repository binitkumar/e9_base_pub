...
class Campaign < ActiveRecord::Base
  ...
  belongs_to :link, :foreign_key => :user_page_id
  ...
  validates :name, :presence     => true,
                   ...
                   :exclusion    => { :in => [NoCampaign::NAME], :unless => lambda {|r| r.is_a?(NoCampaign) } }
    ...
    select(selects).
      ...
      joins(
        'LEFT OUTER JOIN ( ' +
         ...
         "#{ 'AND ' + DatedCost.for_time_range_conditions(*args, options.merge(:column => 'date')).to_sql if args.present?} " +
  ...
  ##
  # Should recreate costs based on the status of the deal, and the costing
  # method of the campaign.  The deal clears its own costs before this method
  # is called.
  #
