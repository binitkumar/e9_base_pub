  ...
  module Model
    ...
    def tag_lists
      {}.tap do |hash|
        tagging_contexts.each do |context|
          hash[context] = tag_list_on(context)
        end
      end
    end

    def copy_tags_from(other)
      other.tagging_contexts.each do |context|
        self.set_tag_list_on(context, other.tag_list_on(context))
      end
    end
    ...
    module DelayedClassMethods
      def tagged_with(tags, options = {})
        retv = super(tags, options.except(:show_all, :show_hidden))
    ...
    included do
      ...
      scope :tagged_with_context, lambda {|context|
        condition = Tagging.arel_table[:context].eq(E9Tags.escape_context(context))
        
        ids = select("DISTINCT #{table_name}.id").joins(:taggings).where(condition)
      ...
      def tags=(tags_or_hash={})
        if tags_or_hash.is_a?(Array)
          tags_or_hash = {'tags' => tags_or_hash}
        end

        self.tag_lists = tags_or_hash
      end
